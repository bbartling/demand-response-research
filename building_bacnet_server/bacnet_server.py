                    ...
                    async with session.get(DR_SERVER_URL) as response:
                        if response.status == 200:
                            ...
                            async with self.server_check_lock:
                                self.current_server_payload = server_data.get(
                                    "payload", 0
                                )
                            _log.info(
                                f"Received cloud DR server response: {self.current_server_payload}"
                            )
        ...
        while True:
            ...
            for req in READ_REQUESTS:
                ...
                try:
                    ...
                    if req["tags"] == "occupancy sensor point":
                        ...
                        occupancy_identifier = ObjectIdentifier(
                        ...
                        _log.debug(
                            "    - self.room_is_occupied: %r", self.room_is_occupied
                        )
            ...
            if DO_WRITES:
                try:
                    ...
                    _log.debug(
                        "hvac_needs_to_be_released status %r",
                        self.hvac_needs_to_be_released,
                    )
